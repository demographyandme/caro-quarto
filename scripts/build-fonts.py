...
Writes <out>/<Face>-{core,latn,sym}.woff2 and <out>/fonts.css, which also
declares metric-matched Georgia fallbacks for Literata and Newsreader (FALLBACKS).
...
# Metric-matched fallbacks. Until a web font arrives the browser sets text in
# the next family of the stack; Georgia runs 7% narrower than Literata and 1-11%
# wider than Newsreader, so text rewrapped when the font swapped in (the home
# page's dictionary definition gained a line on a phone: a 0.12 layout shift).
# Each fallback is Georgia scaled by size-adjust to the web font's width, with
# the web font's vertical metrics. Literata's values are its measured width
# ratio to Georgia on 14.4 px running text (the fleet's body size; bold, which
# Georgia has only one of, at 600, the heading weight). Newsreader's optical
# sizes make its ratio size-dependent (0.99 at 14 px, 0.84 bold at 26-41 px),
# and it sets only titles, headwords and definitions, so its values were chosen
# by measurement: over 10 pages at 412 and 1280 px, the ones that minimised how
# far elements move between fallback and web font (38,349 px of movement with
# plain Georgia, 6,386 with these; the home page on a phone 205 -> 4 px).
# Italics take the same proportion as their roman.
# (fallback family, web face whose metrics it takes, style, weight range,
#  local() names, size-adjust %)
FALLBACKS = [
    ("Literata Fallback",   "Literata",        "normal", "100 499", ("Georgia",),                       106.9),
    ("Literata Fallback",   "Literata",        "normal", "500 900", ("Georgia Bold", "Georgia-Bold"),    95.7),
    ("Literata Fallback",   "Literata-Italic", "italic", "100 499", ("Georgia Italic", "Georgia-Italic"), 98.3),
    ("Literata Fallback",   "Literata-Italic", "italic", "500 900", ("Georgia Bold Italic", "Georgia-BoldItalic"), 89.2),
    ("Newsreader Fallback", "Newsreader",      "normal", "100 499", ("Georgia",),                        98.3),
    ("Newsreader Fallback", "Newsreader",      "normal", "500 900", ("Georgia Bold", "Georgia-Bold"),    84.5),
    ("Newsreader Fallback", "Newsreader",      "italic", "100 499", ("Georgia Italic", "Georgia-Italic"), 95.8),
    ("Newsreader Fallback", "Newsreader",      "italic", "500 900", ("Georgia Bold Italic", "Georgia-BoldItalic"), 82.5),
]
...
FALLBACK_CSS = """@font-face {{
  ...
  font-style: {style};
  size-adjust: {size:.1f}%;
  ascent-override: {ascent:.2f}%;
  descent-override: {descent:.2f}%;
  line-gap-override: {gap:.2f}%;
}}"""

def fallback_css(out):
    """@font-face rules for FALLBACKS; vertical metrics from the built core file,
    divided by size-adjust so the line box matches the web font's."""
    css = []
    for family, face, style, weight, names, size in FALLBACKS:
        f = TTFont(os.path.join(out, f"{face}-core.woff2"))
        upm, hhea = f["head"].unitsPerEm, f["hhea"]
        k = 100.0 / upm / (size / 100.0)
        css.append(FALLBACK_CSS.format(
            family=family, style=style, weight=weight, size=size,
            src=", ".join(f'local("{n}")' for n in names),
            ascent=hhea.ascent * k, descent=-hhea.descent * k, gap=hhea.lineGap * k))
    return css
...
CSS_HEADER = """/* CARO self-hosted typography — generated by scripts/build-fonts.py; do not edit.
   ...
   "Literata Fallback" and "Newsreader Fallback" are Georgia sized to each web
   font, so text does not rewrap when the web font arrives.
